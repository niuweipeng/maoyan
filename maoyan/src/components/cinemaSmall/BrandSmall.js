...
import "../../assets/css/cinemaSmall/brand.css"
import cinemaActionCreatore, {allBrandShow, changeBrandName} from "../../store/action/cinema"
class Brand extends React.Component{
    render(){
        return (
           <div style={{display:this.props.allBrandShow?"block":"none"}} className={"brand2"}>
            <div className={"brandOut2"}>
                {
                    this.props.brand.map(v=>(
                        <div key={v.id} className={"brandIn2"} onClick={()=>{this.props.getBrandById.call(this,v.id,v.name)}}>
                            <div className={"brandInLeft2"}>
                                <span style={{
                                    opacity:v.id===-1?"1":"0"
                                }} className="iconfont icon-duigou"></span>
                                <span className={"brandInLeftRight2"}>{v.name}</span>
                            </div>
                            <div className={"brandInRight2"}>{v.count}</div>
                        </div>
                    ))
                }
            </div>
